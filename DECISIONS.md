...
## D-043 Tenant and operational-fact schema conventions
Every tenant-owned table has a `business_id` column (foreign key, indexed), checked at migration review time — this is the schema-level backstop for the tenancy rules in ARCHITECTURE.md. Immutable operational-fact tables (sales, payments, inventory/cash movements, register closes, cancellations, refunds, audit records) additionally carry: an actor reference (who performed the action), a register/session reference where the action is register-scoped, and correlation/operation IDs for idempotency and cross-record tracing (see the command envelope). Audit and outbox tables are physically separate from operational tables, each indexed for its own query pattern (audit: business + entity + date; outbox: status + created), and are never joined into or read from the POS's live operational queries (see SPECS.md §23.3).

## D-044 Migration policy
Migrations are authored through Prisma Migrate and reviewed like code before being applied to any shared environment (staging/production). Migrations are forward-only: an applied migration is never edited after merge; a mistake is corrected by a new migration, not a rewrite of history — the same append-only principle used for business data applies to schema changes. Destructive changes (dropping a column or table) require a deprecation step — stop writing to/reading from it in application code first, ship and verify that, then drop it in a later migration — rather than one migration that removes something the running application might still reference.

## D-045 Register-closing conflict/override policy
Resolves the "Register-closing conflict/override policy enumeration" item previously listed under Pending decisions. Any user holding `register.override_close_conflict` (seeded to Administrator) may force-close another user's currently open register session, with no additional confirmation step or reason field beyond that permission check — the same shape as every other permission-gated write in this codebase. The override is audited like any other state change (before/after `status`), attributing the close to the overriding user, not the session's original owner. SPECS.md 11.3's "notify the affected user" is satisfied as a simple in-app message shown the next time that user is active, not a persisted notification record (the formal notification module is Phase 8) — a `RegisterSessionsService.close` outside the owner is exactly the affected user's "reopen or reestablish the operational context before continuing": the frontend already re-derives register-session state from the server on every load, so a user whose session was overridden simply lands back on the register selector next time, with no separate re-authentication or acknowledgment flow.
